...
let securePath = '01J2QNPFAHP6HR1XRHTSFFPRCT';
...
export default {
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            securePath = env.XPATH || securePath;
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case `/${securePath}/home`:
                        ...
                        if (!isAuth) return Response.redirect(`${url.origin}/${securePath}/login`, 302);
                    ...
                    case `/${securePath}/login`:
                        ...
                        if (loginAuth) return Response.redirect(`${url.origin}/${securePath}/home`, 302);

                        let secretKey = await env.KV.get('secretKey');
                        const pwd = await env.KV.get('pwd');
                        if (!pwd) await env.KV.put('pwd', 'admin');
                    ...
                    case `/${securePath}/logout`:
                        await env.KV.put('pwd', '');
                        return new Response('Success', {
                            status: 200,
                            headers: {
                    ...
                    default:
                        const notFoundPage = await render404Page();
...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `节点::域名::${index + 1}::端口::${port}`;
            break;
        case 2:
        case 3:
            remark = `节点::IPv4::${index - 1}::端口::${port}`;
            break;
        case 4:
        case 5:
            remark = `节点::IPv6::${index - 3}::端口::${port}`;
            break;
        default:
            remark = `节点::默认IP::${index - 5}::端口${port}`;
            break;
    }
...
const updateDataset = async (env, Settings) => {
    ...
    try {
        currentProxySettings = await env.KV.get("proxySettings", {type: 'json'});
    } catch (error) {
        console.log(error);
...
<head>
  ...
  <title>Dashboard v${panelVersion}</title>
...
<body>
  <h1>Dashboard v<span style="font-size: smaller;">${panelVersion}</span></h1>
        ...
        <tr>
          ...
          <td>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}#Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}#Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/bia-pain-bache/Worker-Panel" target="_blank">Github</a>
	...
	<script>
        ...
        const applySettings = async (event, configForm) => {
            ...
            try {
                ...
                const response = await fetch('/${securePath}/home', {
                    method: 'POST',
                    body: formData,
                    credentials: 'include'
                });
        ...
        const logout = async (event) => {
            ...
            try {
                const response = await fetch('/${securePath}/logout', {
                    method: 'GET',
        ...
        const resetPassword = async (event) => {
            ...
            try {
                const response = await fetch('/${securePath}/password', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'text/plain'
                    },
...
<body>
  ...
  document.getElementById('loginForm').addEventListener('submit', async (event) => {
    ...
    try {
      const response = await fetch('/${securePath}/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'text/plain'
        },
      ...
      if (response.ok) {
        window.location.href = '/${securePath}/home';
      } else {
...
<head>
  ...
  <style id="" media="all">/* latin-ext */
    @font-face {
      ...
      font-weight: 400;
      font-display: swap;
      src: url(https://fonts.gstatic.com/s/chango/v27/2V0cKI0OB5U7WaJCxne5fFU.woff2) format('woff2');
      unicode-range: U+0100-02AF, U+0304, U+0308, U+0329, U+1E00-1E9F, U+1EF2-1EFF, U+2020, U+20A0-20AB, U+20AD-20C0, U+2113, U+2C60-2C7F, U+A720-A7FF;
    }
    /* latin */
    @font-face {
      ...
      font-weight: 400;
      font-display: swap;
      src: url(https://fonts.gstatic.com/s/chango/v27/2V0cKI0OB5U7WaJCyHe5.woff2) format('woff2');
      unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;
    }
    * {
...
<![endif]-->
  ...
  <meta http-equiv="origin-trial" content="">
</head>
...
const renderSvgico = async() => {
    const html = `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="800" viewBox="0 0 32 32" xmlns:v="https://vecta.io/nano"><path d="M15.948 2h.065a10.418 10.418 0 0 1 .972.528q5.429 3.122 10.858 6.246a.792.792 0 0 1 .414.788l-.005 13.164a.813.813 0 0 1-.356.507q-5.773 3.324-11.547 6.644a.587.587 0 0 1-.657.037q-5.78-3.314-11.549-6.64a.7.7 0 0 1-.4-.666q0-6.582 0-13.163a.693.693 0 0 1 .387-.67q5.422-3.118 10.844-6.24L15.948 2" fill="#019639"/><path d="M8.767 10.538q0 5.429 0 10.859a1.509 1.509 0 0 0 .427 1.087 1.647 1.647 0 0 0 2.06.206 1.564 1.564 0 0 0 .685-1.293v-7.86q3.583 4.29 7.181 8.568a2.833 2.833 0 0 0 2.6.782 1.561 1.561 0 0 0 1.251-1.371q.008-5.541 0-11.081a1.582 1.582 0 0 0-3.152 0v7.982l-7-8.332a2.817 2.817 0 0 0-2.649-1.052 1.579 1.579 0 0 0-1.403 1.505z" fill="#fff"/></svg>`;
...
<body>
  <div id="error-container">
    ...
    <div id="error-message">
      <h2>${message} ${refer
                  ? 'Please try again or refer to <a href="https://github.com/bia-pain-bache/Worker-Panel/blob/main/README.md">documents</a>'
                  : ''}
              </h2>
      <p><b>${error ? `${error}` : ''}</b></p>
    </div>
